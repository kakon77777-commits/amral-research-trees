...
* **A gate gets a mutation drill**, and a planted defect must be caught by
  ...
  been green is indistinguishable from a comment. **Position as of RUN-056:
  every gate is drilled — `src00`–`src10` and `src12`–`src58`, 233 defects, 233
  caught by the named check, 45 controls undisturbed, over 84 checks.** RUN-026's
