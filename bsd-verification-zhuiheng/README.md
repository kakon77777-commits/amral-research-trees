...
* **A gate gets a mutation drill**, and a planted defect must be caught by
  ...
  been green is indistinguishable from a comment. **Position as of RUN-058:
  every gate is drilled — `src00`–`src10` and `src12`–`src60`, 243 defects, 243
  caught by the named check, 49 controls undisturbed, over 86 checks.** RUN-026's
