"""Versioned external semantic validator for run-record schema v0.2.6.
...
VALIDATOR_VERSION = "0.2.6"
GATE_VERSION = "i0-admission-gate/0.2.6"
PROJECTION_SPEC_ID = "urn:evemisslab:pnp-glc:candidate-projection:0.2.6"
ARTIFACT_CLOSURE_SPEC_ID = "urn:evemisslab:pnp-glc:artifact-closure:0.2.6"
EVIDENCE_ROLE_SPEC_ID = "urn:evemisslab:pnp-glc:evidence-roles:0.2.6"
TRACE_PRODUCER = "pnp-glc-i0-capability-sandbox"
TRACE_SIGNER_ID = "i0-test-trace-signer-20260809"
TRACE_MEASUREMENT_MODEL = "authenticated-i0-single-worker-linear/0.2.6"
TRACE_SIGNATURE_CONTEXT = b"pnp-glc-i0-trace-v0.2.6\0"
PASS = "pass"
FAIL = "fail"
UNKNOWN = "unknown"
...
PINNED_SCHEMA_HASH = "eb22cb899869b2a43dcd2dbbd3f4c9f7c0a8921ae361ef2207db1039b1c875ee"
PINNED_PROJECTION_SPEC_HASH = "1257968aec544720393b02c9017ac44321dba6521336d6d5a28268045e345c17"
PINNED_CLOSURE_SPEC_HASH = "b635e86cb5fbdb89865ed4343f052247e98a245dd2194726066aef01489f758f"
PINNED_ROLE_SPEC_HASH = "3147f9767711fb46b5d35929b8b9db7219146c5a0d3c15efbd1759bc7dd35109"
PINNED_TRACE_PUBLIC_KEY_HASH = "03ceec52a930415354bc748a0b065f5ff5dc40cfcef6a37ac1e716704addfbaf"
PINNED_SANDBOX_HASH = "6916a2fce0b0c45ac9ba81d2090aedaf51c8c11aa861816a03d65ee7b857609b"
PINNED_RUN_STANDARD_HASH = "850454875011352fa66d67e1815e225712f85c4fec9205a8b052f28e9c055b8b"
PINNED_RUN_ROBUST_HASH = "61c32edbf52d193fc8fea09edc62c69507fe798af57df57dbdd40f806546c9b6"
PINNED_MAXIMAL_HASH = "2c267e417c340f122ca7fb11e479e9dc64836c920a59ae404e1fda4ce958ba29"
PINNED_FAIRNESS_HASH = "0d7702d3926ca278a7e3ed4b5e5c76d65249655d694cf81719cd67372d081710"
PINNED_PARITY_INVARIANT_HASH = "57df1799834067179fb78bbbb5350cadbb402fbe0c78d8cfae6df246947eb34f"
PINNED_PARITY_CONTRACT_HASH = "94c03095cbd3e17ed4389f5df9a153b8e369db3784239e943630327430206fc2"
PINNED_TWO_SAT_CONTRACT_HASH = "f88d798c4a559547159c9def7adba54784d70979edeeebfa941126e3d7a2acee"
...
def _parse_json_int(token: str) -> int:
    if token == "-0":
        raise ValueError("negative zero is outside the v0.2.6 canonical domain")
    return int(token)
    ...
    if isinstance(value, float):
        issues.append(
                ValidationIssue(
                    ...
                    path,
                    "candidate projection v0.2.6 admits integers only",
                )
        )
        return issues
...
def _expected_oracle_declaration(
    family: str, mechanism_id: str, result_status: str
) -> Mapping[str, Any] | None:
    """Derive normative oracle name/checks from the candidate family context."""

    if family == "PARITY":
        if mechanism_id == "parity-stream":
            checks = ["answer", "prefix invariant"]
        if mechanism_id == "parity-table-family":
            checks = ["answer", "answer"]
        if mechanism_id not in {"parity-stream", "parity-table-family"}:
            return None
        return {
    ...
    if family == "2-SAT" and mechanism_id == "2sat-kosaraju":
        if result_status == "sat":
            return {
                "oracle_id": "i0-2sat-assignment-oracle",
                "entrypoint": "pnp_glc_i0.oracles:assignment_satisfies_2sat",
        ...
        if result_status == "unsat":
            return {
                "oracle_id": "i0-2sat-unsat-certificate-oracle",
                "entrypoint": "pnp_glc_i0.oracles:verify_unsat_certificate",
                "name": "independent 2-SAT certificate oracle",
                "obligations": ["mutual-implication-paths"],
                "checks": ["mutual implication paths"],
            }
    return None


def _expected_advice_declaration(
    family: str, mechanism_id: str
) -> Mapping[str, Any] | None:
    """Derive the typed advice/resource declaration for each supported I0 mechanism."""

    if (family, mechanism_id) in {
        ...
        return {
            ...
            "observed_answer_access": "none",
            "resource_profile": "zero",
        }
    if (family, mechanism_id) == ("PARITY", "parity-table-family"):
        return {
            "advice_mode": "per-input-length-truth-table",
            "uniform": False,
            "program_quantifiers": "for-all-lengths-exists-program",
            "advice_generator_ref": PINNED_PARITY_RULE_HASH,
            "declared_answer_access": "truth-table",
            "observed_answer_access": "truth-table",
            "resource_profile": "truth-table-materialized",
        }
    return None


def _advice_declaration_matches(
    record: Mapping[str, Any], trace: Mapping[str, Any]
) -> bool:
    """Check both directions of advice declaration, access, and resource accounting."""

    try:
        family = record["problem"]["family"]
        mechanism_id = record["mechanism"]["id"]
        expected = _expected_advice_declaration(family, mechanism_id)
        if expected is None:
            return False
        admissibility = record["mechanism"]["admissibility"]
        generator_ref = admissibility["advice_generator_ref"]
        normalized_generator = (
            None if generator_ref is None else normalize_hash(generator_ref)
        )
        if (
            admissibility["advice_mode"] != expected["advice_mode"]
            or admissibility["uniform"] is not expected["uniform"]
            or admissibility["program_quantifiers"]
            != expected["program_quantifiers"]
            or normalized_generator != expected["advice_generator_ref"]
            or admissibility["declared_answer_access"]
            != expected["declared_answer_access"]
            or trace.get("answer_access") != expected["observed_answer_access"]
        ):
            return False

        description = record["ledger"]["description_bytes"]
        generation = record["ledger"]["admission_costs"]["advice_generation"]
        if expected["resource_profile"] == "zero":
            return (
                description["advice"] == 0
                and description["generated_tables"] == 0
                and generation["time_ns"] == 0
                and generation["peak_space_bytes"] == 0
                and generation["peak_output_bytes"] == 0
            )

        bits = record["problem"]["generator"]["parameters"]["bits"]
        advice_bytes = TruthTableFamily.build(len(bits)).advice_bytes
        return (
            description["advice"] == advice_bytes
            and description["generated_tables"] == advice_bytes
            and generation["time_ns"] > 0
            and generation["peak_space_bytes"] >= advice_bytes
            and generation["peak_output_bytes"] == advice_bytes
        )
    except (KeyError, TypeError, ValueError):
        return False
...
def _operational_reference_status(
    ...
    if not expected:
        return UNKNOWN, (
            ValidationIssue(
                ...
                "$.mechanism.id",
                "no pinned v0.2.6 operational role map exists for this family/mechanism",
            ),
        )
    ...
    if projection is not None and (
        ...
        issues.append(
            ValidationIssue(
                ...
                "projection spec id/version does not match v0.2.6",
            )
        )
    if closure_spec is not None and (
        ...
        issues.append(
            ValidationIssue(
                ...
                "closure spec id/version does not match v0.2.6",
            )
        )
    expected_edges = {
    ...
    expected_oracle_declaration = _expected_oracle_declaration(
        family,
        mechanism_id,
        record["candidate_result"].get("status"),
    )
    oracle_declaration = record["mechanism"]["oracle"]
    if expected_oracle_declaration is None or any(
        oracle_declaration.get(key) != expected_value
        for key, expected_value in expected_oracle_declaration.items()
    ):
        issues.append(
            ValidationIssue(
                "oracle-declaration-family-binding",
                "$.mechanism.oracle",
                "oracle id/entrypoint/name/checks/obligations do not match the validator-derived family/mechanism/result declaration",
            )
        )

    if not _advice_declaration_matches(record, trace):
        issues.append(
            ValidationIssue(
                "advice-declaration-ledger-binding",
                "$.mechanism.admissibility.advice_mode",
                "typed advice mode must match the validator-derived family/mechanism declaration, generator reference, uniformity/quantifiers, declared and observed answer access, and advice-generation ledger",
            )
        )
    ...
    if any(actual_value != expected_value for actual_value, expected_value in declaration_checks):
        issues.append(
            ValidationIssue(
                ...
                "$.problem",
                "candidate declarations do not match the pinned v0.2.6 family interface",
            )
        )
    ...
    if not isinstance(trace_envelope, dict) or (
        ...
        issues.append(
            ValidationIssue(
                ...
                "trace is not a v0.2.6 capability-trace envelope",
            )
        )
...
def _validate_parsed_record(
    ...
    *,
    ...
    advice_applicable = (
        ...
        or admissibility["advice_generator_ref"] is not None
