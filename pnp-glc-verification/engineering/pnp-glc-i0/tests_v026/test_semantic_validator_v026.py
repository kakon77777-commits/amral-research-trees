...
import pnp_glc_i0.semantic_validator_v026 as validator  # noqa: E402
...
class SemanticValidatorV026Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.schema_path = (
            ROOT / "schemas" / "run-record.schema.v0.2.6-candidate.json"
        )
        cls.schema_bytes = cls.schema_path.read_bytes()
        cls.schema = load_json(cls.schema_path)
        cls.manifest = load_json(ROOT / "fixtures-v0.2.6" / "manifest.json")
    ...
    def test_frozen_predecessors_are_bitwise_preserved(self) -> None:
        expected = {
            ...
            ROOT / "src" / "pnp_glc_i0" / "semantic_validator_v024.py":
                "B744C9C20C510FE39F132E0DFB4AAC50E6E3E573B48B7F1AE19494F5D5195FED",
            ROOT / "SHA256SUMS-v0.2.4-candidate.txt":
        ...
        for name, expected in self.manifest["fixtures"].items():
            with self.subTest(fixture=name):
                report = validate_path(
                    ROOT / "fixtures-v0.2.6" / f"{name}.json",
                    self.schema_path,
                    ROOT,
                ).to_dict()
    ...
    def test_ref_type_receipt_substitution_keeps_signature_but_fails(self) -> None:
        record = load_json(
            ROOT / "fixtures-v0.2.6" / "receipt-ref-substitution.json"
        )
    ...
    def test_valid_signature_cross_role_substitution_fails(self) -> None:
        path = ROOT / "fixtures-v0.2.6" / "cross-role-contract-invariant.json"
    ...
    def test_schema_requires_typed_oracle_identity(self) -> None:
        ...
        baseline = load_json(ROOT / "fixtures-v0.2.6" / "legit.json")
    ...
    def test_advice_declaration_is_bidirectionally_bound(self) -> None:
        coherent_semantic_negatives = {
        ...
        for name in (
            ...
            *sorted(coherent_semantic_negatives),
        ):
            with self.subTest(fixture=name):
                path = ROOT / "fixtures-v0.2.6" / f"{name}.json"
                record = load_json(path)
                trace = self.store.load_json(
                    record["validation_receipt"]["trace_sha256"]
                )
                self.assertEqual(_trace_authenticity_status(record, self.store), PASS)
                self.assertFalse(_advice_declaration_matches(record, trace))
                report = validate_path(path, self.schema_path, ROOT)
                self.assertFalse(report.record_accepted)
                if name in coherent_semantic_negatives:
        ...
        for name in ("legit", "2sat-sat", "2sat-unsat"):
            with self.subTest(positive=name):
                ...
                self.assertTrue(_advice_declaration_matches(record, trace))
                self.assertTrue(validate_path(path, self.schema_path, ROOT).record_accepted)

    def test_schema_replaces_free_text_advice_with_typed_mode(self) -> None:
        checker = Draft202012Validator(self.schema)
        baseline = load_json(ROOT / "fixtures-v0.2.6" / "legit.json")
        missing = copy.deepcopy(baseline)
        missing["mechanism"]["admissibility"].pop("advice_mode")
        self.assertFalse(checker.is_valid(missing))
        legacy = copy.deepcopy(baseline)
        legacy["mechanism"]["admissibility"]["advice"] = "none"
        self.assertFalse(checker.is_valid(legacy))
        contradictory = copy.deepcopy(baseline)
        contradictory["mechanism"]["admissibility"][
            "advice_mode"
        ] = "per-input-length-truth-table"
        self.assertFalse(checker.is_valid(contradictory))

    def test_robust_public_key_type_confusion_fails(self) -> None:
        path = ROOT / "fixtures-v0.2.6" / "robust-ref-type-confusion.json"
        record = load_json(path)
        self.assertEqual(_trace_authenticity_status(record, self.store), PASS)
        report = validate_path(path, self.schema_path, ROOT)
    ...
    def test_supported_api_binds_schema_bytes(self) -> None:
        ...
        record_bytes = (
            ROOT / "fixtures-v0.2.6" / "legit.json"
        ).read_bytes()
    ...
    def test_negative_zero_is_rejected_in_raw_parse_domain(self) -> None:
        report = validate_path(
            ROOT / "fixtures-v0.2.6" / "negative-zero.json",
            self.schema_path,
            ROOT,
        )
        self.assertFalse(report.structural_ok)
    ...
    def test_ill_typed_supported_parameters_fail_closed_without_exception(self) -> None:
        record = load_json(ROOT / "fixtures-v0.2.6" / "legit.json")
    ...
    def test_unpaired_surrogate_has_explicit_scalar_diagnostic(self) -> None:
        report = validate_path(
            ROOT / "fixtures-v0.2.6" / "unpaired-surrogate.json",
            self.schema_path,
            ROOT,
        )
        self.assertIn(
    ...
    def test_malformed_envelope_precedes_unknown_spec_classification(self) -> None:
        ...
        supported_path = ROOT / "artifacts-v0.2.6" / "run-standard.v0.2.6.json"
    ...
    def test_closure_normative_judgment_graph_is_dependency_closed(self) -> None:
        spec = load_json(
            ROOT
            / "artifacts-v0.2.6"
        ...
        self.assertIn(
            "envelope.spec_id equals urn:evemisslab:pnp-glc:artifact-closure:0.2.6",
            header["applicable_when"],
        )
        self.assertIn("envelope.version equals 0.2.6", header["predicate"])
        self.assertEqual(
        ...
        self.assertTrue(opaque_leaf)
        self.assertEqual(
            generic_envelope["false_result"],
            {
        ...
        for name, judgment in judgments.items():
            with self.subTest(judgment=name):
                self.assertIsInstance(judgment.get("depends_on"), list)
                for reference in judgment["depends_on"]:
                    self.assertTrue(reference.startswith("judgments."))
                    self.assertIn(reference.removeprefix("judgments."), judgments)
        self.assertEqual(
            header["depends_on"], ["judgments.GenericEnvelopeShape"]
        )
        self.assertEqual(
            unsupported["depends_on"], ["judgments.GenericEnvelopeShape"]
        )
        self.assertEqual(
            spec["base_envelope_shape"]["normative_status"], "derived-view-only"
        )
    ...
    def test_supported_relation_results_are_normatively_total(self) -> None:
        spec = load_json(
            ROOT
            / "artifacts-v0.2.6"
            / "artifact-closure-spec.v0.2.6.json"
        ...
        traversal = judgments["SupportedTraversal"]

        self.assertEqual(header["false_result"]["gate_result"], "FAIL")
        self.assertTrue(header["false_result"]["terminal"])
        self.assertEqual(
            header["true_result"]["next_transition"],
            "judgments.SupportedEdgeRelation",
        )
        self.assertEqual(relation["false_result"]["gate_result"], "FAIL")
        self.assertTrue(relation["false_result"]["terminal"])
        self.assertEqual(relation["false_result"]["traversal"], "do not traverse")
        self.assertEqual(relation["true_result"]["gate_result"], "PASS")
        self.assertEqual(relation["true_result"]["classification"], "Traverse")
        self.assertEqual(
            relation["true_result"]["next_transition"],
            "judgments.SupportedTraversal",
        )
        self.assertEqual(
            traversal["depends_on"], ["judgments.SupportedEdgeRelation"]
        )
        self.assertEqual(
            {
                value["gate_result"]
                for value in traversal["fixed_point_results"].values()
            },
            {"PASS", "FAIL", "UNKNOWN"},
        )
        self.assertTrue(
    ...
    def test_schema_gate_assignment_conformance(self) -> None:
        cheat = copy.deepcopy(load_json(ROOT / "fixtures-v0.2.6" / "cheat.json"))
        cheat["validation_receipt"]["gates"][
            "advice_generation_pass"
        ] = "not-applicable"
        self.assertFalse(Draft202012Validator(self.schema).is_valid(cheat))

        legit = copy.deepcopy(load_json(ROOT / "fixtures-v0.2.6" / "legit.json"))
    ...
    def test_all_gate_applicability_mutations_are_schema_rejected(self) -> None:
        ...
        for fixture_name in ("legit", "robust-legit", "neutral-legit"):
            baseline = load_json(
                ROOT / "fixtures-v0.2.6" / f"{fixture_name}.json"
            )
