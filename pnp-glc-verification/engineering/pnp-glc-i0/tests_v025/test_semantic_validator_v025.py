...
import pnp_glc_i0.semantic_validator_v025 as validator  # noqa: E402
...
class SemanticValidatorV025Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.schema_path = (
            ROOT / "schemas" / "run-record.schema.v0.2.5-candidate.json"
        )
        cls.schema_bytes = cls.schema_path.read_bytes()
        cls.schema = load_json(cls.schema_path)
        cls.manifest = load_json(ROOT / "fixtures-v0.2.5" / "manifest.json")
        ...
        for name, expected in self.manifest["fixtures"].items():
            with self.subTest(fixture=name):
                report = validate_path(
                    ROOT / "fixtures-v0.2.5" / f"{name}.json",
                    self.schema_path,
                    ROOT,
                ).to_dict()
    ...
    def test_ref_type_receipt_substitution_keeps_signature_but_fails(self) -> None:
        record = load_json(
            ROOT / "fixtures-v0.2.5" / "receipt-ref-substitution.json"
        )
    ...
    def test_valid_signature_cross_role_substitution_fails(self) -> None:
        path = ROOT / "fixtures-v0.2.5" / "cross-role-contract-invariant.json"
    ...
    def test_oracle_declaration_is_bound_to_problem_family(self) -> None:
        for name in (
            ...
            with self.subTest(fixture=name):
                path = ROOT / "fixtures-v0.2.5" / f"{name}.json"
                record = load_json(path)
                self.assertEqual(_trace_authenticity_status(record, self.store), PASS)
                self.assertEqual(
    ...
    def test_schema_requires_typed_oracle_identity(self) -> None:
        ...
        baseline = load_json(ROOT / "fixtures-v0.2.5" / "legit.json")
    ...
    def test_robust_public_key_type_confusion_fails(self) -> None:
        path = ROOT / "fixtures-v0.2.5" / "robust-ref-type-confusion.json"
        record = load_json(path)
        self.assertEqual(_trace_authenticity_status(record, self.store), PASS)
        report = validate_path(path, self.schema_path, ROOT)
    ...
    def test_supported_api_binds_schema_bytes(self) -> None:
        ...
        record_bytes = (
            ROOT / "fixtures-v0.2.5" / "legit.json"
        ).read_bytes()
    ...
    def test_negative_zero_is_rejected_in_raw_parse_domain(self) -> None:
        report = validate_path(
            ROOT / "fixtures-v0.2.5" / "negative-zero.json",
            self.schema_path,
            ROOT,
        )
        self.assertFalse(report.structural_ok)
    ...
    def test_ill_typed_supported_parameters_fail_closed_without_exception(self) -> None:
        record = load_json(ROOT / "fixtures-v0.2.5" / "legit.json")
    ...
    def test_unpaired_surrogate_has_explicit_scalar_diagnostic(self) -> None:
        report = validate_path(
            ROOT / "fixtures-v0.2.5" / "unpaired-surrogate.json",
            self.schema_path,
            ROOT,
        )
        self.assertIn(
    ...
    def test_malformed_envelope_precedes_unknown_spec_classification(self) -> None:
        ...
        supported_path = ROOT / "artifacts-v0.2.5" / "run-standard.v0.2.5.json"
    ...
    def test_closure_normative_judgment_graph_is_dependency_closed(self) -> None:
        ...
        self.assertIn(
            "envelope.spec_id equals urn:evemisslab:pnp-glc:artifact-closure:0.2.5",
            header["applicable_when"],
        )
        self.assertIn("envelope.version equals 0.2.5", header["predicate"])
        self.assertEqual(
    ...
    def test_schema_gate_assignment_conformance(self) -> None:
        cheat = copy.deepcopy(load_json(ROOT / "fixtures-v0.2.5" / "cheat.json"))
        cheat["validation_receipt"]["gates"][
            "advice_generation_pass"
        ] = "not-applicable"
        self.assertFalse(Draft202012Validator(self.schema).is_valid(cheat))

        legit = copy.deepcopy(load_json(ROOT / "fixtures-v0.2.5" / "legit.json"))
    ...
    def test_all_gate_applicability_mutations_are_schema_rejected(self) -> None:
        ...
        for fixture_name in ("legit", "robust-legit", "neutral-legit"):
            baseline = load_json(
                ROOT / "fixtures-v0.2.5" / f"{fixture_name}.json"
            )
