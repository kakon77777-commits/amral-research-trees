...
from pnp_glc_i0.semantic_validator_v026 import (  # noqa: E402
...
VALIDATOR = ROOT / "src" / "pnp_glc_i0" / "semantic_validator_v026.py"
ARTIFACTS = ROOT / "artifacts-v0.2.6"
FIXTURES = ROOT / "fixtures-v0.2.6"
PARENT_ARTIFACTS = ROOT / "artifacts-v0.2.5"
PARENT_FIXTURES = ROOT / "fixtures-v0.2.5"
TRACES = ARTIFACTS / "traces"
AUTH = ARTIFACTS / "auth"
PROJECTION_SPEC = ARTIFACTS / "candidate-projection-spec.v0.2.6.json"
CLOSURE_SPEC = ARTIFACTS / "artifact-closure-spec.v0.2.6.json"
ROLE_SPEC = ARTIFACTS / "evidence-role-spec.v0.2.6.json"
PUBLIC_KEY = ARTIFACTS / "trace-public-key.v0.2.6.json"
SANDBOX = ARTIFACTS / "capability-sandbox.v0.2.6.json"
RUN_STANDARD = ARTIFACTS / "run-standard.v0.2.6.json"
RUN_ROBUST = ARTIFACTS / "run-robust.v0.2.6.json"
MAXIMAL = ARTIFACTS / "maximal-run.v0.2.6.json"
FAIRNESS = ARTIFACTS / "fairness.v0.2.6.json"
INVARIANT = ARTIFACTS / "parity-invariant.v0.2.6.json"
PARITY_CONTRACT = ARTIFACTS / "contract-parity.v0.2.6.json"
TWO_SAT_CONTRACT = ARTIFACTS / "contract-2sat.v0.2.6.json"
CLOSURE_CLASS_FIXTURES = ARTIFACTS / "closure-classification"
...
def write_closure_classification_fixtures() -> dict[str, Path]:
    """Materialize the generic EnvelopeShape classification matrix."""
    ...
    cases: dict[str, tuple[dict[str, Any], str]] = {
        "unsupported-missing-spec-id": (
            {"artifact_type": "run-spec", "version": "9.0.0", "edges": []},
            "fail",
        ),
        "unsupported-missing-artifact-type": (
            {"spec_id": unsupported, "version": "9.0.0", "edges": []},
            "fail",
        ),
        "unsupported-missing-version": (
            {"spec_id": unsupported, "artifact_type": "run-spec", "edges": []},
            "fail",
        ),
        "unsupported-missing-edges": (
            {
                "spec_id": unsupported,
                "artifact_type": "run-spec",
                "version": "9.0.0",
        ...
        "unsupported-ill-typed-spec-id": (
            {**valid, "spec_id": 9},
            "fail",
        ),
        "unsupported-ill-typed-artifact-type": (
            {**valid, "artifact_type": ["run-spec"]},
            "fail",
        ),
        "unsupported-ill-typed-version": (
            {**valid, "version": 9},
            "fail",
        ),
        "unsupported-ill-typed-edges": (
            {**valid, "edges": {}},
            "fail",
        ),
        "unsupported-empty-spec-id": ({**valid, "spec_id": ""}, "fail"),
        "unsupported-empty-artifact-type": (
            {**valid, "artifact_type": ""},
            "fail",
        ),
        "unsupported-empty-version": ({**valid, "version": ""}, "fail"),
        "unsupported-malformed-edge-member": (
            {
                **valid,
                "edges": [
                    {"role": "future-edge", "expected_type": "opaque-content"}
                ],
            },
            "fail",
        ),
        "unsupported-malformed-edge-hash": (
            {**valid, "edges": [{**valid_edge, "sha256": "not-a-hash"}]},
            "fail",
        ),
        "unsupported-duplicate-edge-role": (
            {**valid, "edges": [valid_edge, dict(valid_edge)]},
            "fail",
        ),
        "shape-valid-unsupported-known-type": (valid, "unknown"),
        "shape-valid-unsupported-future-type": (
            {**valid, "artifact_type": "future-artifact", "edges": [valid_edge]},
            "unknown",
        ),
        "supported-wrong-version": (
            {**supported, "version": "9.0.0"},
            "fail",
        ),
        "supported-future-artifact-type": (
            {**supported, "artifact_type": "future-artifact", "edges": [valid_edge]},
            "fail",
        ),
        "supported-invalid-edge-relation": (
            {**supported, "edges": [valid_edge]},
            "fail",
        ),
    }
    paths: dict[str, Path] = {}
    expected: dict[str, str] = {}
    for name, (envelope, status) in cases.items():
        path = CLOSURE_CLASS_FIXTURES / f"{name}.json"
        write_json(path, {"artifact_envelope": envelope})
        paths[name] = path
        expected[name] = status
    write_json(
        CLOSURE_CLASS_FIXTURES / "manifest.json",
        {
            "classification": "Experiment fixtures for CLOSURE-CLASS-01; no P/NP conclusion",
            "policy": "complete generic EnvelopeShape is validated before supported/unsupported spec dispatch",
...
def version_candidate(
    ...
    record["recorded_at"] = "2026-08-10T09:00:00Z"
    record["provenance"]["sources"][0].update(
        {
            "id": "run-record-v0.2.6-candidate",
            "locator": "schemas/run-record.schema.v0.2.6-candidate.json",
    ...
    for claim in record["claims"]:
        claim["source_refs"] = ["run-record-v0.2.6-candidate"]
    ...
    source_advice_mode = admissibility.get("advice_mode")
    mechanism_id = record["mechanism"]["id"]
    if mechanism_id == "parity-table-family":
        if source_advice_mode != "per-input-length-truth-table":
            raise ValueError("unexpected parent advice declaration for table family")
        admissibility["advice_mode"] = "per-input-length-truth-table"
    else:
        if source_advice_mode != "none":
            raise ValueError("unexpected parent advice declaration for uniform mechanism")
        admissibility["advice_mode"] = "none"

    if record["problem"]["family"] == "PARITY":
        record["mechanism"]["oracle"].update(
            {
                "oracle_id": "i0-parity-oracle",
                "entrypoint": "pnp_glc_i0.oracles:parity_oracle",
                "name": "independent parity oracle",
                "obligations": (
                    ["answer", "prefix-invariant"]
        ...
        oracle_status = record["candidate_result"]["status"]
        record["mechanism"]["oracle"].update(
            {
                "oracle_id": (
                    "i0-2sat-assignment-oracle"
                    if oracle_status == "sat"
                    else "i0-2sat-unsat-certificate-oracle"
                ),
                "entrypoint": (
                    "pnp_glc_i0.oracles:assignment_satisfies_2sat"
                    if oracle_status == "sat"
                    else "pnp_glc_i0.oracles:verify_unsat_certificate"
                ),
                "name": "independent 2-SAT certificate oracle",
                "obligations": (
                    ["assignment"]
                    if oracle_status == "sat"
                    else ["mutual-implication-paths"]
                ),
                "checks": (
                    ["assignment"]
                    if oracle_status == "sat"
...
def parity_candidate(
    ...
    parent_name = {
        (False, False, True): "legit",
        (True, False, True): "cheat",
        (False, True, True): "robust-legit",
        (False, False, False): "neutral-legit",
        (False, True, False): "robust-neutral-legit",
    }.get((cheat, robust, bounded))
    if parent_name is None:
        raise ValueError("unsupported parent parity template")
    record = json.loads(
        (PARENT_FIXTURES / f"{parent_name}.json").read_text(encoding="utf-8")
    )
    trace = json.loads(
        (PARENT_ARTIFACTS / "traces" / f"{parent_name}.trace.json").read_text(
            encoding="utf-8"
        )
    )
...
def two_sat_candidate(
    ...
    if name not in {"2sat-sat", "2sat-unsat"}:
        raise ValueError("unsupported parent 2-SAT template")
    record = json.loads(
        (PARENT_FIXTURES / f"{name}.json").read_text(encoding="utf-8")
    )
    trace = json.loads(
        (PARENT_ARTIFACTS / "traces" / f"{name}.trace.json").read_text(
            encoding="utf-8"
        )
    )
    parameters = record["problem"]["generator"]["parameters"]
    if parameters != {"variable_count": variable_count, "clauses": clauses}:
        raise ValueError("parent 2-SAT template does not match requested instance")
    return version_candidate(record, trace)
...
def gate_values(record: Mapping[str, Any], trace: Mapping[str, Any]) -> dict[str, str]:
    ...
    advice_declared = (
        record["mechanism"]["admissibility"]["advice_mode"]
    ...
    return {
        ...
        "advice_generation_pass": PASS if advice_declared else NOT_APPLICABLE,
...
def main() -> None:
    ...
    closure_case_paths = write_closure_classification_fixtures()
    ...
    # ADVICE-DECL-LEDGER-01: typed mode is bidirectionally bound to the
    # mechanism context, generator, uniformity/quantifiers, answer access, and
    # advice-generation resource fold.
    item, attack_trace = clone_candidate(
        records["legit"],
        traces["legit"],
        "advice-table-with-null-generator-zero-ledger",
    )
    item["mechanism"]["admissibility"][
        "advice_mode"
    ] = "per-input-length-truth-table"
    negative["advice-table-with-null-generator-zero-ledger"] = finalize_fixture(
        "advice-table-with-null-generator-zero-ledger",
        item,
        attack_trace,
        private_key,
    ...
    item["mechanism"]["admissibility"]["advice_mode"] = "none"
    negative["advice-none-with-table-generator-ledger"] = finalize_fixture(
        "advice-none-with-table-generator-ledger",
        item,
        attack_trace,
        private_key,
    ...
    table_admissibility = records["cheat"]["mechanism"]["admissibility"]
    for field in (
        "advice_mode",
        "uniform",
        "program_quantifiers",
        ...
        item["mechanism"]["admissibility"][field] = copy.deepcopy(
            table_admissibility[field]
        )
    for field in ("advice", "generated_tables"):
        item["ledger"]["description_bytes"][field] = records["cheat"]["ledger"][
            "description_bytes"
        ][field]
    item["ledger"]["admission_costs"]["advice_generation"] = copy.deepcopy(
        records["cheat"]["ledger"]["admission_costs"]["advice_generation"]
    )
    attack_trace["answer_access"] = "truth-table"
    attack_trace["resource_samples"]["description_bytes"] = copy.deepcopy(
        item["ledger"]["description_bytes"]
    )
    attack_trace["resource_samples"]["admission_costs"] = copy.deepcopy(
        item["ledger"]["admission_costs"]
    )
    negative["parity-stream-with-coherent-table-advice"] = finalize_fixture(
        "parity-stream-with-coherent-table-advice",
        item,
        attack_trace,
        private_key,
    ...
    none_admissibility = records["legit"]["mechanism"]["admissibility"]
    for field in (
        "advice_mode",
        "uniform",
        "program_quantifiers",
        ...
        item["mechanism"]["admissibility"][field] = copy.deepcopy(
            none_admissibility[field]
        )
    for field in ("advice", "generated_tables"):
        item["ledger"]["description_bytes"][field] = records["legit"]["ledger"][
            "description_bytes"
        ][field]
    item["ledger"]["admission_costs"]["advice_generation"] = copy.deepcopy(
        records["legit"]["ledger"]["admission_costs"]["advice_generation"]
    )
    attack_trace["answer_access"] = "none"
    attack_trace["resource_samples"]["description_bytes"] = copy.deepcopy(
        item["ledger"]["description_bytes"]
    )
    attack_trace["resource_samples"]["admission_costs"] = copy.deepcopy(
    ...
    negative["parity-table-with-coherent-none-advice"] = finalize_fixture(
        "parity-table-with-coherent-none-advice",
        item,
        attack_trace,
        private_key,
    ...
    # ORACLE-DECL-FAMILY-01: the source hash is intentionally shared, but the
    # normative name/checks declaration must still be derived from the family.
    item, attack_trace = clone_candidate(
        records["legit"], traces["legit"], "parity-with-2sat-oracle-declaration"
    )
    item["mechanism"]["oracle"] = copy.deepcopy(
        records["2sat-sat"]["mechanism"]["oracle"]
    )
    negative["parity-with-2sat-oracle-declaration"] = finalize_fixture(
    ...
    item["mechanism"]["oracle"] = copy.deepcopy(
        records["legit"]["mechanism"]["oracle"]
    )
    negative["2sat-with-parity-oracle-declaration"] = finalize_fixture(
        "2sat-with-parity-oracle-declaration",
        item,
        attack_trace,
        private_key,
    ...
    item["mechanism"]["oracle"] = copy.deepcopy(
        records["2sat-unsat"]["mechanism"]["oracle"]
    )
    negative["2sat-sat-with-unsat-oracle-declaration"] = finalize_fixture(
        "2sat-sat-with-unsat-oracle-declaration",
        item,
        attack_trace,
        private_key,
    ...
    negative["2sat-unsat-with-sat-oracle-declaration"] = finalize_fixture(
        "2sat-unsat-with-sat-oracle-declaration",
        item,
        attack_trace,
        private_key,
    ...
    donor_oracle = records["2sat-sat"]["mechanism"]["oracle"]
    for field in ("oracle_id", "entrypoint", "name", "checks", "obligations"):
        attack_name = f"parity-with-2sat-oracle-{field}-only"
        item, attack_trace = clone_candidate(
            records["legit"], traces["legit"], attack_name
        )
        item["mechanism"]["oracle"][field] = copy.deepcopy(donor_oracle[field])
        negative[attack_name] = finalize_fixture(
            attack_name,
            item,
            attack_trace,
            private_key,
    ...
    item, attack_trace = clone_candidate(
        records["legit"], traces["legit"], "malformed-unsupported-envelope"
    )
    negative["malformed-unsupported-envelope"] = finalize_fixture(
        "malformed-unsupported-envelope",
        item,
        attack_trace,
        private_key,
        receipt_overrides={
            "run_spec_ref": hash_ref(
                closure_case_paths["unsupported-missing-artifact-type"]
            )
        },
        sign_actual_operational_map=True,
    )
    ...
    item, attack_trace = clone_candidate(
        records["legit"], traces["legit"], "shape-valid-unsupported-envelope"
    )
    negative["shape-valid-unsupported-envelope"] = finalize_fixture(
        ...
        receipt_overrides={
            "run_spec_ref": hash_ref(
                closure_case_paths["shape-valid-unsupported-known-type"]
            )
        },
        sign_actual_operational_map=True,
    )
    ...
    structural_rejected = {
        ...
        "advice-table-with-null-generator-zero-ledger",
        "advice-none-with-table-generator-ledger",
        "declared-answer-access-mismatch",
    }
    ...
    write_json(
        ...
        {
            "schema": "schemas/run-record.schema.v0.2.6-candidate.json",
            "validator": "src/pnp_glc_i0/semantic_validator_v026.py",
