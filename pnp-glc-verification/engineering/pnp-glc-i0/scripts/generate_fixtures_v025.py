...
from pnp_glc_i0.semantic_validator_v025 import (  # noqa: E402
...
SCHEMA = ROOT / "schemas" / "run-record.schema.v0.2.5-candidate.json"
VALIDATOR = ROOT / "src" / "pnp_glc_i0" / "semantic_validator_v025.py"
ARTIFACTS = ROOT / "artifacts-v0.2.5"
FIXTURES = ROOT / "fixtures-v0.2.5"
TRACES = ARTIFACTS / "traces"
AUTH = ARTIFACTS / "auth"
PROJECTION_SPEC = ARTIFACTS / "candidate-projection-spec.v0.2.5.json"
CLOSURE_SPEC = ARTIFACTS / "artifact-closure-spec.v0.2.5.json"
ROLE_SPEC = ARTIFACTS / "evidence-role-spec.v0.2.5.json"
PUBLIC_KEY = ARTIFACTS / "trace-public-key.v0.2.5.json"
SANDBOX = ARTIFACTS / "capability-sandbox.v0.2.5.json"
RUN_STANDARD = ARTIFACTS / "run-standard.v0.2.5.json"
RUN_ROBUST = ARTIFACTS / "run-robust.v0.2.5.json"
MAXIMAL = ARTIFACTS / "maximal-run.v0.2.5.json"
FAIRNESS = ARTIFACTS / "fairness.v0.2.5.json"
INVARIANT = ARTIFACTS / "parity-invariant.v0.2.5.json"
PARITY_CONTRACT = ARTIFACTS / "contract-parity.v0.2.5.json"
TWO_SAT_CONTRACT = ARTIFACTS / "contract-2sat.v0.2.5.json"
...
def version_candidate(
    ...
    record["provenance"]["sources"][0].update(
        {
            "id": "run-record-v0.2.5-candidate",
            "locator": "schemas/run-record.schema.v0.2.5-candidate.json",
    ...
    for claim in record["claims"]:
        claim["source_refs"] = ["run-record-v0.2.5-candidate"]
    ...
    source_advice = admissibility.pop("advice", None)
    mechanism_id = record["mechanism"]["id"]
    if mechanism_id == "parity-table-family":
        if source_advice != "one truth table per n":
            raise ValueError("unexpected source advice declaration for table family")
        admissibility["advice_mode"] = "per-input-length-truth-table"
    else:
        if source_advice != "none":
            raise ValueError("unexpected source advice declaration for uniform mechanism")
...
def main() -> None:
    ...
    write_json(
        ...
        {
            "schema": "schemas/run-record.schema.v0.2.5-candidate.json",
            "validator": "src/pnp_glc_i0/semantic_validator_v025.py",
