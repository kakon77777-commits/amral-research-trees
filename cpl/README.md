...
| `CPL_Claude_67_25_Research_Pack_v2_2026-08-11` … `v7` (v2–v7) | Scaffolding toward reconstructing the paper's own 68.185% certificate ceiling. Found the exact adjacent constant `0.681828687463832…` by reading the official Lean source directly. Built a from-scratch toy linear-programming model that reproduces the paper's "open-band adversarial" argument structure; an N=4 exact-rational toy bound reached 69.82110925% in v5, self-labeled **"not a Riemann zeta theorem."** **v6 is the version that first exact-rationally crosses 70%** in this same N=4 toy model (`B_cert = 11254781/3068556 ≈ 3.667777612662112`, exact Bernstein-subdivision dual certificate over all three multiplicity patterns), still explicitly a toy marked-configuration theorem, not a claim about the real Fourier support constant (the paper's own route needs support ≈1.04 for 70%, which this toy result does not reach or claim to). v7 refines that certificate further. A full N=256 external certificate was never obtained through v7. |
...
`v1 base` → `v2` → `v3` → `v4` → `v5` → `v6` → `v7` → `v8` → `v9` → `v10` → `v11`. Read v1's `00_README.md` first regardless — every later version assumes its `P_q` definition and the "proven / open / forbidden-to-extrapolate" status vocabulary it sets up.

**Note on v6's provenance:** unlike v2–v5/v7–v11, the original `Downloads/CPL/` folder had no standalone `v6` zip — it only existed nested inside `v7`'s own `CPL_Claude_Research_Pack_v6_2026-08-11.zip`. Extracted out to its own top-level directory here on import so it isn't silently skipped like the other packages' *legitimately* redundant nested prior-version zips (which are left in place — see e.g. `v9/CPL_Claude_Research_Pack_v8_2026-08-11.zip` — because they duplicate a sibling top-level directory already in this tree).
