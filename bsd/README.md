...
| **P5 — single-prime strong BSD for 389.a1 at p=11** | `BSD_P5_Norm_Selmer_Core_Vertex_v1.2`, `BSD_P5_Determinantal_Kurihara_Semilocal_v1.3`, `BSD_P5_ETNC_Escape_Audit_v0.4`, `BSD_P5_Explicit_Local_Unit_Cancellation_v0.8`, `BSD_P5_IMC_Closure_and_GPR_Bridge_v0.5`, `BSD_P5_uGPR_Minimal_Gate_v0.6`, `BSD_RUGZPB_P2_P4_Rank2_v0.2` (7 packages) | Curve 389.a1 (rank 2) at prime p=11. `Sha[11^∞] = 0` is closed; several sub-lemmas are closed. The **core target — bridging the analytic leading term to the algebraic regulator at p=11 — remains OPEN across every package**, most recently stated explicitly: "does not prove BSD and does not prove local descent of the normalized complex leading coefficient." Not yet curated on the public site. |
...
4. P5 packages: `Norm_Selmer_Core_Vertex_v1.2` → `Determinantal_Kurihara_Semilocal_v1.3` is the main spine; the other five are supporting audits/gates for that spine.
...
Original source: `C:\Users\kakon\Downloads\BSD\` (25 items: 24 `.zip` + 1 loose `.md`, 92MB extracted). Each zip's own internal folder-nesting has been flattened by one level on import here (each zip extracts to `<name>/<name>/...`; this repo keeps `<name>/...`). No other content transformation was applied.
