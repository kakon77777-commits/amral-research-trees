...
P01_MUTATIONS = [
    ("L01-predecessor-modulus", "the odd-predecessor rule is tested against 4 mod 8",
     "        if has_odd_pred != (n % 6 == 4):",
     "        if has_odd_pred != (n % 8 == 4):",
     ["P01_S4_1_odd_predecessor_iff_n_is_4_mod_6"]),
    ("L02-density", "the branch-point density is claimed as 1/3",
     "          abs(hits / 60000 - 1 / 6) < 1e-4",
     "          abs(hits / 60000 - 1 / 3) < 1e-4",
     ["P01_S4_2_branch_point_density_is_one_sixth"]),
    ("L03-power-of-two-test", "the power-of-two test also accepts multiples of 3",
     "            if x & (x - 1) == 0:",
     "            if x % 3 == 0:",
     ["P01_S5_reaching_one_iff_meeting_a_power_of_two"]),
    ("L04-rotation", "the rotation drops the detached subtree",
     "    return (l_label, LL, (label, LR, R))",
     "    return (l_label, LL, (label, None, R))",
     ["P01_S111_tree_rotation_preserves_the_node_set"]),
    ("L05-ggm-threshold", "the GGM threshold is evaluated as p^(p-1)",
     "          p ** Fraction(p, p - 1) == 4)",
     "          p ** Fraction(p - 1, p) == 4)",
     ["P01_GGM_threshold_at_p_equals_2_is_exactly_4"]),
    ("L06-citation-title", "a cited title is altered, so the arXiv record no longer matches",
     '        "title": "The 3x+1 Semigroup",',
     '        "title": "The 3x+2 Semigroup",',
     ["P01_all_cited_arxiv_records_match_title_authors_and_journal"]),
    ("L07-doi", "the Barina DOI is checked against the wrong volume",
     '        and cr["volume"] == "81"',
     '        and cr["volume"] == "80"',
     ["P01_barina_2025_doi_matches_the_stated_journal_volume_and_article"]),
    ("NULL-09", "control: a comment is added",
     "def norm(s: str) -> str:",
     "# control mutation, no behavioural change\ndef norm(s: str) -> str:",
     []),
]
...
TARGETS = [
    ...
    ("code/ot_paper01_recheck.py", P01_MUTATIONS, []),
