"""RUN-054 mutation drill for `src73_unit_sync.py`.
...
in `src73_drill_defects.py`; several of its anchors carry embedded quotes and
...
Usage:
    python code/src73_drill.py --bundle <dir>
"""
...
from src73_drill_defects import DEFECTS                        # noqa: E402

ROOT = pathlib.Path(__file__).resolve().parent.parent
GATE = ROOT / "code" / "src73_unit_sync.py"
...
def _complaints(res: dict) -> list[str]:
    return (list(res.get("failures", []))
            ...
            + list(res.get("counters_not_in_the_failure_or_population_lists",
                           []))
            + list(res.get(
                "artifact_defects_the_mathematics_does_not_depend_on", [])))
...
def main() -> int:
    ...
    for name, addition in CONTROLS:
        try:
            ...
            write_gate(snapshot)
        # the baseline is not complaint-free here: the bundle carries a real
        # artifact defect. A control must leave the verdict UNCHANGED, which
        # is not the same as leaving it empty.
        report["controls"][name] = {
            "undisturbed": (bool(res.get("passed"))
                            and _complaints(res) == _complaints(base)),
