...
| `check_profile_registry.py` | Refuses if the profiles this tree implements and the SEDB registry at `SEDB/projects/amral-results-profiles/` disagree, if the registry has drifted from this tree's measurement, or if the local mirror has drifted from the live export. An unreachable registry is reported as `unmeasured`, never as agreement. |
| `check_profile_registry_drill.py` | Drills the above, including the state that is easiest to get wrong: with nothing readable the verdict must be `unmeasured` and not ok. |
